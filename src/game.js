  ...
  class LostLightScene extends Phaser.Scene {
    ...
    makeTextures() {
      const makePerson = (key, coat, hair = COLORS.ink) => {
        ...
        g.fillStyle(hair).fillRect(2, 0, 4, 2);
        g.fillStyle(COLORS.cream).fillRect(2, 2, 4, 3);
        g.fillStyle(coat).fillRect(1, 5, 6, 5);
        g.fillStyle(COLORS.ink).fillRect(1, 10, 2, 2).fillRect(5, 10, 2, 2);
        g.generateTexture(key, 8, 12);
        g.destroy();
      };
      makePerson("hero", COLORS.deep, 0x5b352d);
      MENTORS.forEach(m => makePerson(m.id, m.color));
      const flame = this.make.graphics({ add: false });
      flame.fillStyle(COLORS.red).fillRect(2, 3, 4, 4);
      flame.fillStyle(COLORS.gold).fillRect(3, 1, 2, 5);
      flame.fillStyle(COLORS.white).fillRect(3, 3, 2, 3);
      flame.generateTexture("flame", 8, 8);
      flame.destroy();
    }
    ...
    buildInn() {
      ...
      const g = this.add.graphics();
      g.fillStyle(COLORS.deep).fillRect(0, 0, WIDTH, HEIGHT);
      for (let y = 2; y < 15; y++) {
        for (let x = 1; x < 19; x++) {
      ...
      g.fillRect(8, 16, 144, 5);
      g.fillRect(8, 118, 144, 4);
      g.fillRect(8, 16, 4, 106);
      g.fillRect(148, 16, 4, 106);
      ...
      this.npcs = MENTORS.map(m => {
        const sprite = this.add.sprite(m.x * TILE + 4, m.y * TILE + 2, m.id).setDepth(9);
      ...
      const innkeeper = {
        ...
        sprite: this.add.sprite(9 * TILE + 4, 4 * TILE + 2, "fighter").setTint(0xc99157).setDepth(9)
      };
    ...
    drawFurniture(g) {
      ...
      rect(14, 12, 3, 1);
      rect(8, 8, 2, 1);
      g.fillStyle(COLORS.gold).fillRect(76, 61, 8, 8);
      this.add.sprite(80, 65, "flame").setDepth(3);
    }
    ...
    showChoice(npc) {
      this.choicePanel = this.add.graphics().setDepth(55);
